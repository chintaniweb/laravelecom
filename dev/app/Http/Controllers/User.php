...
class User extends Controller {
    ...
    public function create() {
        ...
        //fetch data for boces role 
        $user_role_boces = Role::where('website_id', boces_website_id)->lists('name', 'id');
        //fetch data for cte role
        $user_role_cte = Role::where('website_id', cte_website_id)->lists('name', 'id');
        //fetch data for eta role
        $user_role_eta = Role::where('website_id', eta_website_id)->lists('name', 'id');
        //load view
        return view('User.create', ['user_create' => $user_create, 'user_role_boces' => $user_role_boces, 'user_role_cte' => $user_role_cte,'user_role_eta'=>$user_role_eta]);
    }
    ...
    public function store(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            //fetch eta role id
            $eta_role_id = $user['eta_role_id'];
            $role_eta_data = explode(" ", $eta_role_id);
            
            //merge boces,cte & eta
            $data = array_merge($role_boces_data, $role_cte_data,$role_eta_data);
    ...
    public function edit($id) {
       ...
       //fetch location data from location tabe
        $boces_role =  Role::where('website_id', boces_website_id)->lists('name', 'id')->all();
        $cte_role =  Role::where('website_id', cte_website_id)->lists('name', 'id')->all();
        $eta_role =  Role::where('website_id', eta_website_id)->lists('name', 'id')->all();

        return view('User.edit', ['user' => $user, 'website' => $website,'boces_role'=> $boces_role,'cte_role'=>$cte_role,'eta_role'=>$eta_role]);
    }
