...
<div class="container">
    <div class="row">
        <div class="col-xs-12 col-sm-5">
            <h4 class="page-title">Update Filing Cabinet</h4>
        </div>
    </div>
    <div class="row">
        <div class="row m-0">
        ...
        {!! Form::model($data,array('route' => array('Filing_cabinet.update',$data->filing_cabinet_id), 'method' => 'PUT','files'=>'true','class' => 'form-horizontal','id'=>'myForm')) !!}
        ...
        <div class="col-xs-12 col-sm-12 m-t-20">
            ...
            <div class="col-sm-7 col-md-8">
                <div class="card-box m-b-0">
                    <div class="row">
                        <div class=" col-sm-12 col-xs-12 p-t-b-10">
                            <div class="form-group row form-main-box">
                                <label class="control-label col-md-3" for="site">Upload</label>
                                <div class="col-md-7">
                                    {!! Form::file('file_name',['class'=>'btn btn-block btn-grey', 'autocomplete' => 'off']),null!!}
                                </div>
                                <?php $file_name = (isset($data->file_name) && ($data->file_name != "")) ? $data->file_name : ""; ?>
                                <?php if (isset($file_name) && ($file_name != "")) { ?>
                                    <a target="_blank" href="<?php echo url('/') . "/resources/views/Filing_cabinet/filing_cabinet_files/" . $file_name; ?>">View</a>
                                    <a href="<?php echo url('/') . "/Filing_cabinet/deleteImage/" . $data->filing_cabinet_id; ?>" onclick="return confirm('Are you sure you want to delete this image?');">|Delete</a>
                                <?php } ?>
                                <span id="err_link_url" class="error col-md-7 col-md-push-4">{{$errors->first('file_name')}}</span>
                            </div>

                            <div class="form-group row form-main-box">
                                {!!Form::label('Start Date','Start Date',['class'=>'control-label col-md-3'])!!}   
                                <div class="col-md-7">
                                    {!! Form::hidden('show_file_date',null,['class' => 'form-control','id' => 'hidden_show_file_date']) !!}
                                    <div id='show_file_date' name="show_file_date">&nbsp;</div>
                                </div>
                            </div>
                            <div class="form-group row form-main-box">
                                {!!Form::label('End Date','End Date',['class'=>'control-label col-md-3'])!!}
                                <div class="col-md-7">
                                    {!! Form::hidden('hide_file_date',null,['class' => 'form-control','id' => 'hidden_hide_file_date']) !!}
                                    <div id='hide_file_date' name="hide_file_date">&nbsp;</div>
                                    news item will display as active until 11:59 PM ET of this date
                                </div>
                            </div>
                            <div class="form-group row form-main-box">
                                <label class="control-label col-md-3">File Description<i data-tooltip="tooltip" title="file_description" data-original-title="File Description"></i></label>
                                <div class="col-md-7">
                    ...
                    {!!Form::close()!!}
                    {!! Form::open(array('url' => 'Filing_cabinet/'.$data->filing_cabinet_id)) !!}
                    {!! Form::hidden('_method', 'DELETE') !!}
                    {!! Form::submit('Delete', array('class' => 'btn btn-primary btn-rect btn-sm','onclick' => 'return confirm("Are you sure you want to delete this item?")')) !!} Filing Cabinet File
                    {!!Form::close()!!}
